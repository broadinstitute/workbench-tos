...
// mock authorizer that always returns successfully. Normally the authorizer requires an auth token,
// but this mock expects a userinfo object, and echoes that userinfo object back. We do this so
// different unit tests can specify different users, which is important for the datastore mocks below.
                        ...
                        mutationResults: [
                            {
                                key: {
                                    ...
                                    partitionId: {
                                        projectId: 'unit-test', // broad-dsde-dev or the env it's running in
                                        namespaceId: 'app',
                                    },
                                },
    ...
    return {
        ...
        headers: {
            ...
            authorization: `{"user_id": ${userid}, "email": "fake@fakey.fake", ` +
                '"verified_email": true, "audience": 122333444455555, "expires_in": 500}',
        },
        method: 'GET',
        query: {
            appid: appid,
...
const postRequest = function(userid, appid = 'FireCloud', tosversion = 20180815.1,
    accepted = true, email = 'fake@fakey.fake') {
...
test("GET userresponse: should reject with a 404 if user's TOSResponse doesn't exist", async t => {
    ...
    const error = await t.throwsAsync(tosapi(req, res,
        new SuccessfulMockAuthorizer(), new UnitTestDatastoreClient()));
    t.is(error.statusCode, 404);
    t.is(error.name, 'ResponseError');
    t.is(error.message, 'Error reading user response: 404');
});

test("GET userresponse: should reject with a 403 and 'user declined TOS' " +
    "if user's TOSResponse has accepted: false", async t => {
...
test("GET userresponse: should reject with a 403 and 'user declined TOS' " +
    "if user's TOSResponse is missing an accepted value", async t => {
...
test("GET userresponse: should reject with a 500 and 'unexpected: returned too many results' " +
    'if query somehow returns multiple TOSResponses for the same user', async t => {
...
test("GET userresponse: should reject with a 500 and 'unexpected error' " +
    'if the query truly throws an unexpected error', async t => {
...
test("GET userresponse: should reject with a 500 and the rejection's contents " +
    'if the query truly throws an unexpected rejection', async t => {
...
test('GET userresponse: should return TOSResponse object/success ' +
    'if user has exactly 1 TOSresponse and it has accepted:true', async t => {
